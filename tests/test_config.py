...
def test_loads_repo_config():
    ...
    assert labels == {"32": "Strassenverkehrsamt", "61": "Strassenverkehrsamt", "62": "Wallisellen"}


def test_display_defaults_and_overrides():
    cfg = load_config(REPO_CONFIG)
...
def test_missing_label_falls_back_to_stripped_city(tmp_path):
    p = tmp_path / "c.json"
...
def test_rejects_negative_min_time(tmp_path):
    p = tmp_path / "c.json"
    p.write_text(json.dumps({
