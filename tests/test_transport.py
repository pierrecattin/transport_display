...
def _board():
    return json.loads(FIXTURE.read_text(encoding="utf-8"))["stationboard"]


def test_minutes_until_floors_and_clamps():
...
def test_parse_matches_number_and_direction_only():
...
def test_visible_departures_applies_min_time():
    deps = parse_departures(_board(), CONNS)
...
def test_parse_skips_malformed_entries():
    board = _board() + [{"number": "32"}, "garbage", {"number": "32", "to": "Zürich, Strassenverkehrsamt"}]
